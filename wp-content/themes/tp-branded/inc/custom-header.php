 ...
 *
 * @uses tp_branded_header_style()
 */
function tp_branded_custom_header_setup() {
	add_theme_support( 'custom-header', 
		apply_filters( 'tp_branded_custom_header_args', 
			array(
				...
				'height'                 => 528,
				'flex-height'            => true,
				'wp-head-callback'       => 'tp_branded_header_style',
			)
		)
	);
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'tp-branded' ),
		),
	) );
}
add_action( 'after_setup_theme', 'tp_branded_custom_header_setup' );

if ( ! function_exists( 'tp_branded_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see tp_branded_custom_header_setup().
	 */
	function tp_branded_header_style() {
		...
		wp_add_inline_style( 'tp-branded-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'tp_branded_header_style', 10 );