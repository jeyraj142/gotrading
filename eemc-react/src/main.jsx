...
import {Zap,Calculator,BookOpen,Search,Home,Star,Clock3,FileText,Menu,ChevronLeft,Heart,Lightbulb,Activity,TowerControl,Network,ShieldAlert,Boxes,Settings2,Cable,Grounding, Gauge, Wrench, RefreshCcw, ChevronDown, CheckCircle2} from 'lucide-react';
...
['Earthing',Grounding,'c9'],['Measurements',Gauge,'c10'],['Electrician Tools',Wrench,'c11'],['Unit Conversion',RefreshCcw,'c12']
];
...
function App(){
 ...
 return <div className="app-shell">
  {screen==='home'?<>
   ...
   <main>
    <div className="search"><Search size={21}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search calculators, formulas, topics..."/></div>
    <div className="modes">
      <button className={'mode quick '+(mode==='Quick'?'on':'')} onClick={()=>setMode('Quick')}><Zap/><b>Quick Mode</b><small>Fast Calculations</small></button>
      <button className={'mode eng '+(mode==='Engineer'?'on':'')} onClick={()=>setMode('Engineer')}><Settings2/><b>Engineer Mode</b><small>Detailed & Professional</small></button>
      <button className={'mode learn '+(mode==='Learning'?'on':'')} onClick={()=>setMode('Learning')}><BookOpen/><b>Learning Mode</b><small>Concepts & Examples</small></button>
    </div>
    ...
    <div className="section-title"><h3>{category||'Popular Calculators'}</h3><span>{filtered.length} tools</span></div>
    <div className="calc-list">{filtered.slice(0,12).map(f=><button key={f.id} onClick={()=>openCalc(f)}><div className="mini-icon"><Zap size={18}/></div><div><b>{f.title}</b><span>{f.cat}</span></div><span className="go">›</span></button>)}</div>
   </main>
  ...
  </>:<CalculatorScreen f={selected} mode={mode} onBack={()=>setScreen('home')} fav={fav} setFav={setFav}/>} 
 </div>
}
...
function CalculatorScreen({f,mode,onBack,fav,setFav}){
 ...
 return <div className="calc-screen">
   <div className="calc-top"><button onClick={onBack}><ChevronLeft/></button><b>{f.title}</b><button onClick={()=>setFav(!fav)}><Heart fill={fav?'currentColor':'none'}/></button></div>
